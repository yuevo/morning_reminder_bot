    // 23行目のシート名を今月のシートに修正、68行目の第4引数を今月の最終日に修正

    // WebhookURLを追加
    let postUrl = "ここにWebhookURLを追加する";　
  
    // botを投入したいチャンネル名を追加
    let postChannel = "#ここにチャンネル名を入れる";
...
　　function morningRemind(){
    ...
    // 今日の日付から参加者リストを取得
    let join_lists = sheet.getSheetValues(3, 9 + row, 14, 1);
    // メンバー全員分のユーザIDを取得
    ...
    let meet_url = "meet.google.com/xfx-mcsj-xon"
    
    sendHttpPost_('おはようございます！朝活リマインダーbotです:sunny:\
                  \n' + reminder +'\
                  \n今日の朝活参加者リストです！\
                  \n参加の方は【 7時30分 】までにこちらへどうぞ！' + meet_url + '','朝活リマインダーbot',':chicken:');
